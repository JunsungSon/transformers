...
@add_start_docstrings("""Bert Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layers on top of
                      ...
                      BERT_START_DOCSTRING,
                      BERT_INPUTS_DOCSTRING)
class BertForHotPotQA(BertPreTrainedModel):
    r"""
        **start_positions**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`).
    ...
    """
    def __init__(self, config):
        super(BertForHotPotQA, self).__init__(config)
        self.num_labels = config.num_labels
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)
        self.s_f_classifier = nn.Linear(config.hidden_size, 2) # supporting fact classifier

        self.init_weights()

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None,
                start_positions=None, end_positions=None, labels=None, sf_labels=None):
        ...
        sf_logits = self.s_f_classifier(pooled_output)
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss += loss_fct(sf_logits.view(-1, 2), sf_labels.view(-1))
#            outputs = (loss,) + outputs
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_loss = loss_fct(start_logits, start_positions)
            end_loss = loss_fct(end_logits, end_positions)
            qa_total_loss = (start_loss + end_loss) / 2
#            outputs = (total_loss,) + outputs

        if loss is not None:
            if qa_total_loss is not None:
                loss += qa_total_loss
            outputs = (loss,) + outputs
