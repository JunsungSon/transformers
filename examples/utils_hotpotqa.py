...
class HotPotQAExample(object):
    """
    A single training/test example for the HotPotQA dataset.
    ...
    def __init__(self,
        ...
        self.is_supporting_fact = is_supporting_fact
    ...
    def __repr__(self):
        ...
        if self.has_answer:
            s += ", has_answer: %r" % (self.has_answer)
        if self.is_supporting_fact:
            s += ", has_answer: %r" % (self.is_supporting_fact)
        return s
...
def read_hotpotqa_examples(input_file, is_training):
    """Read a HotPotQA json file into a list of HotPotQAExample."""
    ...
    for entry in input_data:
        ...
        for supporting_fact in entry["supporting_facts"]:
            if supporting_facts.get(supporting_fact[0]) is None:
                supporting_facts[supporting_fact[0]] = []
            supporting_facts[supporting_fact[0]].append(supporting_fact[1])

        for context in entry["context"]:
            title = context[0]
            paragraphs = context[1]
            num_list = []
            if supporting_facts.get(title) is not None:
                num_list = supporting_facts[title]
            for i, paragraph_text in enumerate(paragraphs):
                try:
                    is_supporting_fact = (num_list.index(i) >= 0)
                except ValueError:
                    ...
                    if is_supporting_fact:
                        ...
                        if answer_offset >= 0:
                            ...
                            if actual_text.find(cleaned_answer_text) == -1:
                                logger.warning("Could not find answer: '%s' vs. '%s' - '%s'",
                                               actual_text, cleaned_answer_text, orig_answer_text)
                                continue
                    else:
                        start_position = -1
                        end_position = -1
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
                                 doc_stride, max_query_length, is_training, label_list=None,
    ...
    for (example_index, example) in enumerate(tqdm(examples)):
        ...
        if is_training and not example.has_answer:
            tok_start_position = -1
            tok_end_position = -1
        if is_training and example.has_answer:
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            span_has_answer = example.has_answer
            start_position = None
            end_position = None
            if is_training and span_has_answer:
            ...
            if is_training and not span_has_answer:
...
RawResult = collections.namedtuple("RawResult",
                                   ["unique_id", "start_logits", "end_logits", "cls_logits", "sf_logits"])
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    all_cls_predictions = collections.OrderedDict()
    all_nbest_json = collections.OrderedDict()
    all_sp_predictions = collections.OrderedDict()
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        cls_count = [0, 0, 0]
        cls_max_pred = [0, 0, 0]
        cls_max_text = ["yes", "no", "span"]
        sf_count = [0, 0]
        sf_max_pred = [0, 0]
        for (feature_index, feature) in enumerate(features):
            result = unique_id_to_result[feature.unique_id]

            cls_pred = result.cls_logits.detach().cpu().numpy()
            sf_pred = result.sf_logits.detach().cpu().numpy()

            cls = np.argmax(cls_pred)
            sf = np.argmax(sf_pred)
...
#                all_nbest_json[example.qas_id] = nbest_json
            ...
            if sf_count[sf] == 0:
                sf_max_pred[sf] = sf_pred[sf]
            sf_count[sf] += 1
            if sf_max_pred[sf] < sf_pred[sf]:
                sf_max_pred[sf] = sf_pred[sf]
        ...
        if sf_count[0] == sf_count[1]:
            sf_max = np.argmax(sf_max_pred)
        else:
        ...
        all_cls_predictions[example.qas_id]["text"][cls_max] = cls_max_text[cls_max]

    all_predictions["sp"] = all_sp_predictions
